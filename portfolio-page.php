	...
	if ( ! empty( $pf_group_exists ) ) :
		...
		if ( $r->have_posts() ) :
			...
			<ul id="filters">
				...
				} // For the default portfolio group make the label more readable
				?>
				<li class="active">
					<a href="javascript:void(0)" class="all"><?php echo ucwords( strtolower( $parent_name ) ); ?></a>
				</li>

				<?php foreach ( $cats as $cat ) { ?>
					<li>
						<a href="javascript:void(0)" class="<?php echo $cat->slug; ?>"><?php echo ucwords( strtolower( $cat->cat_name ) ); ?></a>
					</li>
				<?php } ?>
			...
			<div id="pc-portfolio"<?php echo $pf_container_class; ?>>
				...
				<ul class="filterable-grid">
					...
					<?php while ( $r->have_posts() ) : $r->the_post(); ?>
						...
						$char_limit = 60; /* Limit title to certain number of chars. */

						/* Trim title if char limit exceeded. */
