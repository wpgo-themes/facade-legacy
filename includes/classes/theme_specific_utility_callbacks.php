...
if ( ! class_exists( 'PC_TS_Utility_Callbacks' ) ) :
	...
	class PC_TS_Utility_Callbacks {
		...
		public function set_solid_header_theme_option_field() {
			...
			?>
			<div class="box">
				<label><input name="<?php echo PC_OPTIONS_DB_NAME; ?>[chk_solid-header-bg]" type="checkbox" value="1" class="alignleft" <?php if ( isset( $options['chk_solid-header-bg'] ) ) {
						checked( '1', $options['chk_solid-header-bg'] );
					} ?> /> Use solid header bg color
