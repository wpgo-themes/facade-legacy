...
if ( isset( $_POST['cf_submitted'] ) ) {
	...
	if ( trim( $_POST['cf_email'] ) === '' ) {
		...
		if ( ! eregi( "^[A-Z0-9._%-]+@[A-Z0-9._%-]+\.[A-Z]{2,4}$", trim( $_POST['cf_email'] ) ) ) {
							...
							<form action="<?php the_permalink(); ?>" id="contactForm" method="post">
								<div class="contactform">
									<div class="cffield">
										...
										<input type="text" name="cf_contactName" id="cf_contactName" value="<?php if ( isset( $_POST['cf_contactName'] ) ) {
											echo $_POST['cf_contactName'];
									...
									<div class="cffield">
										...
										<input type="text" name="cf_email" id="cf_email" value="<?php if ( isset( $_POST['cf_email'] ) ) {
											echo $_POST['cf_email'];
									...
									<div class="cffield">
										...
										<textarea name="cf_comments" id="cf_message" rows="20" cols="30" class="requiredField"><?php if ( isset( $_POST['cf_comments'] ) ) {
												if ( function_exists( 'stripslashes' ) ) {
													echo stripslashes( $_POST['cf_comments'] );
												} else {
													echo $_POST['cf_comments'];
												}
											} ?></textarea>
									...
									<div class="cfcheckbox">
										<input type="checkbox" name="cf_sendCopy" id="cf_sendCopy" value="true"<?php if ( isset( $_POST['cf_sendCopy'] ) && $_POST['cf_sendCopy'] == true ) {
											echo ' checked="checked"';
										} ?> /><label for="cf_sendCopy"><?php _e( 'Send yourself a copy of this email?', 'presscoders' ); ?></label>
									</div>
									<div class="cfsubmit">
										<input type="hidden" name="cf_submitted" id="cf_submitted" value="true" /><input class="submit button" type="submit" value="<?php _e( 'Send E-mail', 'presscoders' ); ?>" />
									</div>
								</div>
							</form>
